...
export default {
	...
	props: {
		...
		uploadUrl: {
			type: String,
			default: '/api/mt/shop/public/shopUpload'
		}
	},
	watch: {
		isClear(val) {
	...
	methods: {
		seteditor() {
			...
			// 	Authorization: localStorage.getItem('juhezuoToken'),
			// }; // 自定义 header
			...
			this.editor.config.uploadImgHooks = {
				...
				customInsert: (insertImg, result, editor) => {
					// 图片上传成功，插入图片的回调
					if (result.code == 0) {
            console.log("serviceUrl:",this.$envConfig.serviceUrl)
						let url = this.$envConfig.serviceUrl + result.result;
            console.log("url:",url)
						insertImg(url);
					}
				},
