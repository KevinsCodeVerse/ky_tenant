  ...
  <div id="">
    <main-head>
      <el-form :inline="true" class="demo-form-inline" size="medium" :disabled="load">
        <el-form-item>
            <el-select placeholder="售后状态" v-model="params.afterSellStatus" clearable @change="search">
                <el-option v-for="(item,index) in afterSellStatusList" :key="index" :value="item.value" :label="item.label"></el-option>
            </el-select>
        </el-form-item>
        <el-form-item>
          <el-input
              placeholder="输入订单编号搜索"
              v-model="params.afterOrderId"
              clearable>
          </el-input>
        </el-form-item>
        <el-form-item>
          <el-input
              placeholder="输入售后编号搜索"
              v-model="params.afterNo"
              clearable>
          </el-input>
        </el-form-item>
        <el-form-item>
    ...
    <main-content>
      ...
      <el-table :data="list" v-loading="loading" stripe fixed="right" :height="tableHeight" ref="tableWrapper">
        <el-table-column label="订单编号" prop="orderId" align="center" width="120px"></el-table-column>
        <el-table-column label="售后编号" prop="afterNo" align="center" width="120px"></el-table-column>
        <el-table-column label="用户信息" prop="userName" align="center" width="200px"></el-table-column>
        <el-table-column label="售后类型" prop="afterNo" align="center">
          <template slot-scope="scope">
            {{ scope.row.afterSellType == 1?'仅退款':'退货退款'}}
          </template>
        </el-table-column>
        <el-table-column label="售后原因" prop="reason" align="center" width="200px" show-overflow-tooltip></el-table-column>
        ...
        <!--          </template>-->
        <!--        </el-table-column>-->
        <el-table-column label="售后状态" align="center" min-width="180px">
          <template slot-scope="scope">
            <div v-html="getAfterStatus(scope.row.afterSellStatus)"></div>
          </template>
        </el-table-column>
        <el-table-column label="订单状态" align="center" min-width="180px">
          <template slot-scope="scope">
        ...
        <el-table-column label="申请售后时间" align="center" width="200px">
          <template slot-scope="scope">
            {{ $moment(scope.row.afterCreateTime).format('Y-MM-DD HH:mm:ss') }}
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="205px" fixed="right">
          <template slot-scope="scope">
            <el-button size="small" type="text" v-if="scope.row.afterSellStatus == 1" @click="updateStatus(scope.row.afterId, 1)">通过</el-button>
            <el-button size="small" type="text" v-if="scope.row.afterSellStatus == 1" @click="updateStatus(scope.row.afterId, -1)">拒绝</el-button>
              ...
              <div v-for="(item, index) in detailFrom.afterList" :key="index">
                  ...
                  <div>
                    <el-descriptions  :column="3" border>
                      ...
                      <el-descriptions-item label="售后类型">{{item.userOrderAfter.afterSellType==1?'仅退款':'退货退款'}}
  ...
  methods: {
    //审核状态
      updateStatus(id, status) {
          ...
          // } else {
              //成功
              this.$confirm('此操作将'+(status==1?'通过':'拒绝')+'售后, 是否继续?', '提示', {
                  confirmButtonText: '确定',
                  cancelButtonText: '取消',
                  type: 'success'
              })
      ...
      updateStatus2(id, status, refusedReason){
          ...
          this.loading = true;
          this.$request.post({
              url: 'tenant/order/auditAfterSale',
              params: {
                  id,
                  type: status,
                  refusedReason
    ...
    getAfterStatus(e) {
      switch (e) {
        case 0:
          return '<span style="color: #3498db">无售后</span>'
        case 1:
          return '<span style="color: #e67e22">待审核</span>'
        case 2:
          return '<span style="color: #7f8c8d">拒绝售后</span>'
        case 3:
          return '<span style="color: #19BE6B">通过售后</span>'
      }
    },
    getStatus(e) {
      switch (e) {
