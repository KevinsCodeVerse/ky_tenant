	...
	<div id="authority_role">
        ...
        <main-content>
          <template v-slot:top>
            ...
            <!-- 表格 -->
            <el-table :data="list" stripe style="width: 100%" v-loading="load" height="530">
                ...
                <el-table-column label="操作" align="center" width="310" class-name="operation">
                    <template slot-scope="scope">
                        <div >
                            <el-button type="text" plain size="small" @click="getMenuData(scope.row.id)">分配菜单</el-button>
                            <el-button type="text" plain size="small" @click="setRoleForm(scope.row)">修改</el-button>
                            <el-button type="text" plain size="small" @click="del(scope.row.id, scope.row.accountNum)" style="color: #e74c3c">删除</el-button>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
    ...
    <main-footer>
      <el-pagination
		...
		<el-dialog title="分配菜单" :visible.sync="menuVisible" width="600px" center @close="closeDialog">
			<span slot="footer" class="dialog-footer">
				<el-tree :data="menuData" show-checkbox node-key="id" ref="menu" highlight-current :check-strictly="true">
				</el-tree>
		...
		methods: {
			...
			freeze(id) {
				...
				this.$confirm('是否冻结该角色?', '提示', {
					...
					this.load = true
					this.$request.post({
						url: 'tenant/tenantRole/freezeOrThaw',
						params: {
							id,
							status: -1
						},
			...
			getMenuData(id) {
				...
				this.roleId = id;
				this.$request.post({
					url: "tenant/tenantRole/getRoleMenuId",
					params: {
						roleId: id
					},
					success: result => {
						this.$refs.menu.setCheckedKeys(result);
						// this.$refs.menu.setCheckedKeys([20]);
					}
				});
			},
			...
			setMenuData() {
				...
				this.$request.post({
					url: "tenant/tenantRole/setRoleMenu",
					type: "array",
					params: {
						roleId: this.roleId,
						menuIds
