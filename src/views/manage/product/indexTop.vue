...
<main-content>
  <el-table :data="list" v-loading="loading"  fixed="right" @selection-change="handleSelectionChange" :row-key="getRowKeys"
            :header-cell-style="tableHeaderColor" height="570">
    ...
    <el-table-column label="状态" align="center" width="90px">
      <template slot-scope="scope">
        <el-button v-if="scope.row.status == 2" type="info" size="small">仓库中</el-button>
        <el-button v-if="scope.row.status == 1" type="success" size="small">销售中</el-button>
      </template>
    </el-table-column>
...
<!--          <el-button type="text" size="mini" @click="remove(scope.row.id)" style="color: #e74c3c">删除</el-button>-->
          <el-button type="text" size="mini" @click="cancelIndex(scope.row.id)" v-if="scope.row.indexRecommend===1">取消推荐</el-button>
